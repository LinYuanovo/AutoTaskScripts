...
功能: 签到、浏览、预约、查询积分
变量: syns_token = 'Accesstoken' (7.wawo.cc域名请求中的Accesstoken值)
定时: 一天两次
cron: 10 11,12 * * *
------------更新日志------------
...
class AutoTask:
    ...
    def check_env(self):
        ...
        try:
            # 从环境变量获取cookie
            syns_token = os.getenv(f"syns_token")
            if not syns_token:
                self.log("[检查环境变量] 没有找到环境变量syns_token，请检查环境变量", level="error")
                return None

            # 自动检测分隔符
            split_char = None
            for sep in MULTI_ACCOUNT_SPLIT:
                if sep in syns_token:
                    split_char = sep
                    break
            if not split_char:
                # 如果都没有分隔符，默认当作单账号
                syns_tokens = [syns_token]
            else:
                syns_tokens = syns_token.split(split_char)

            for syns_token in syns_tokens:
                yield syns_token
        except Exception as e:
        ...
        try:
            ...
            for index, token in enumerate(self.check_env(), 1):
                ...
                if not self.get_user_info(session):
                    self.log(f"[{self.nickname}] 获取用户信息失败，跳过该账号", level="error")
                    continue
                # 签到
