...
class AutoTask:
    ...
    def check_env(self):
        ...
        try:
            # 从环境变量获取cookie
            yqftfxh = os.getenv(f"yqftfxh")
            if not yqftfxh:
                self.log("[检查环境变量] 没有找到环境变量yqftfxh，请检查环境变量", level="error")
                return None
            ...
            if not split_char:
                ...
                yqftfxhs = yqftfxh.split(split_char)

            for yqftfxh in yqftfxhs:
                if "=" in yqftfxh:
                    yqftfxh = yqftfxh.split("=")[1]
                    yield yqftfxh
                else:
                    yield yqftfxh
        except Exception as e:
