...
功能: 签到、查积分
变量: nyg_token = 'token' (app.niuyougu.com.cn域名下请求中的token值)
定时: 一天两次
cron: 10 8,9 * * *
------------更新日志------------
...
class AutoTask:
    ...
    def check_env(self):
        ...
        try:
            # 从环境变量获取cookie
            nyg_token = os.getenv(f"nyg_token")
            if not nyg_token:
                self.log("[检查环境变量] 没有找到环境变量nyg_token，请检查环境变量", level="error")
                return None
            ...
            if not split_char:
                # 如果都没有分隔符，默认当作单账号
                nyg_tokens = [nyg_token]
            else:
                nyg_tokens = nyg_token.split(split_char)

            for nyg_token in nyg_tokens:
                if "=" in nyg_token:
                    nyg_token = nyg_token.split("=")[1]
                    yield nyg_token
                else:
                    yield nyg_token
        except Exception as e:
    ...
    def get_user_info(self, session, log=False):
        ...
        try:
            ...
            if int(response_json['code']) == 0:
                ...
                self.log(f"[{self.nickname}] 获取用户信息: 发生错误: {response_json['msg']}", level="error")
                return False
        except Exception as e:
