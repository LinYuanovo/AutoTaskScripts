...
class AutoTask:
    ...
    def check_env(self):
        ...
        try:
            # 从环境变量获取cookie
            lbfwwsc_openid = os.getenv(f"lbfwwsc_openid")
            if not lbfwwsc_openid:
                self.log("[检查环境变量] 没有找到环境变量lbfwwsc_openid，请检查环境变量", level="error")
                return None

            # 自动检测分隔符
            split_char = None
            for sep in MULTI_ACCOUNT_SPLIT:
                if sep in lbfwwsc_openid:
                    split_char = sep
                    break
            if not split_char:
                # 如果都没有分隔符，默认当作单账号
                lbfwwsc_openids = [lbfwwsc_openid]
            else:
                lbfwwsc_openids = lbfwwsc_openid.split(split_char)

            for lbfwwsc_openid in lbfwwsc_openids:
                if "=" in lbfwwsc_openid:
                    lbfwwsc_openid = lbfwwsc_openid.split("=")[1]
                    yield lbfwwsc_openid
                else:
                    yield lbfwwsc_openid
        except Exception as e:
